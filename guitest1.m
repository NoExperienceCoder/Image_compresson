...
% Last Modified by GUIDE v2.5 30-Jul-2021 20:44:05
...
% handles    structure with handles and user data (see GUIDATA)
handles.output = hObject;
delta = str2double(get(handles.edit1, 'string'));
if delta>1 || delta <0
    errordlg('Delta must be a value between 0 and 1');
end
pixel = str2double(get(handles.edit3, 'string'));
[filename,pathname]=uigetfile({'*.jpg';'*.jpeg';'*.png';'*.*'},'File Selector');
if ~filename
    errordlg('Select an Image File.');
     return;
end
FileCall(filename,pathname,delta,pixel);
guidata(hObject, handles);
